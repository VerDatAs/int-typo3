...
function trackVideoEvents(videoObj, cExtentions) {
  ...
  function tve(videoObj, cExtentions) {
    ...
    if (
      ...
      enableVideoEcharts && enableAuthorLA
    ) {
      videoObj
        .closest(".ce-container")
        .insertAdjacentHTML(
          "beforeend",
          "<div id = 'container_" +
            cid +
      ...
      echarts08("", "container_" + cid, "", vKey, "dark", videoObj);
    }
    observer = new window.IntersectionObserver(
      ([entry]) => {
            ...
            if (vSource.includes("youtube")) {
              ...
              videoObj.contentWindow.postMessage(
                JSON.stringify({
                  ...
                  func: "seekTo",
                  args: [
                    sessionStorage.getItem("videoCurrTime___" + vSource),
                    true
                  ]
                }),
                "*"
              ...
              videoObj.contentWindow.postMessage(
                {
                  ...
                  value: sessionStorage.getItem("videoCurrTime___" + vSource)
                },
                "*"
              );
            } else {
              ...
              sessionStorage.setItem("cid", cid);
              sessionStorage.setItem("vSource", vSource);
              videoObj.currentTime = sessionStorage.getItem(
                "videoCurrTime___" + vSource
              );
            }
          }
        ...
        if (vSource) {
          ...
          if (typeof result !== "undefined" && result.extensions["https://w3id.org/xapi/video/extensions/current-time"])
    ...
    function sendPaused(curTime, duration, visited, progress, cx) {
      ...
      if (progress < 1) {
        let vc = "videoCurrTime___" + sessionStorage.getItem("vSource");
        if (videoType === "youtube") sessionStorage.setItem(vc, curTime);
        else if (videoType === "vimeo") sessionStorage.setItem(vc, curTime);
        else sessionStorage.setItem(vc, curTime);
